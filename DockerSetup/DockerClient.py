...
def get_links(url, timeout=5):
    ...
    try:
        ...
        # Encode the page to base64 as the website might have excape characters
...
def update_url(new_url: list[str], new_page: list[str], id: int, old_url: str = "") -> None:
    """
    ...
    # Remove this block later, create fake data for pages for development


    server_url =  ServerIP + "/update"
    payload = {'url': new_url, 'page': new_page, 'old_url': old_url, 'Client': id}
    try:
        response = requests.post(server_url, json=payload)
        if response.status_code == 200:
            #print(response.text)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                for i in pages:
                    pagePayload.append(i)
            except TypeError:
                pass
            start_time = time.perf_counter()
            update_url(linkPayload, pagePayload, id, url)
